...
function [mask, fg_eroded] = fgErodedSP ( outline, radius, superpixels )
% Compute the eroded foreground mask of an outline.
...
function [mask, skeleton] = skeletonSP ( outline, radius_threshold, superpixels )
...
function [mask, time_gc, nIter, skeleton, fixed_fg] = grabcut ( image, outline, radius_threshold, superpixels, method, constraint, constraint_bg)
% Compute the segmentation mask of an outline obtained with GrabCut
	...
	if strcmp(method,'skeleton')
		[fixed_fg, skeleton] = Outline.skeletonSP ( outline, radius_threshold, superpixels );
	elseif strcmp(method,'erosion')
		[fixed_fg, skeleton] = Outline.fgErodedSP ( outline, radius_threshold, superpixels );
	elseif strcmp(method, 'none')
		fixed_fg = [];
		constraint = 'none';
	end

	bg_constraint.type = constraint_bg;
